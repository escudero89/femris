...
var G_SHAPES_WIDTH = parseInt($G_DRAW_SHAPES.css("width")) * 0.9;
...
function transformCoordinates(xnode, factorOfDeformation) {
    ...
    alpha = ( G_SHAPES_WIDTH / ( x_max - x_min ) );
    beta  = G_SHAPES_WIDTH * ( 1/10*0 - x_min / ( x_max - x_min ) ) + G_SHAPES_WIDTH * .05;
...
var domainObject = {
    ...
    drawElements : function () {
        ...
        for ( var k = 0 ; k < this.ielem.length ; k++ ) {
            ...
            if ( this.options && this.options.localParamsTextSVG ) {
                ...
                var fontSize = paramsTextSVG['font-size'];
                paramsTextSVG['font-size'] = fontSize * 1.2;
    ...
    drawNodes : function () {
        ...
        for ( var j = 0; j < this.xnode.length; j++ ) {
            ...
            twoNode = this.two.makeCircle(this.xnode[j][0], this.xnode[j][1], G_SHAPES_WIDTH * 0.04);
